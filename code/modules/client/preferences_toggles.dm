...
/client/verb/toggle_scanner_to_chat()
	set name = "Toggle Scans to Chat"
	set category = "Preferences"
	set desc = ".Toggle between outputting scan results to chat and into a seperate window."

	prefs.analyzis_to_chat = !prefs.analyzis_to_chat
	to_chat(src, "You will now be getting analyzis results sent [prefs.analyzis_to_chat ? "to chat" : "to a seperate window"].")
	prefs.save_preferences()
	feedback_add_details("admin_verb","TSTC") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
